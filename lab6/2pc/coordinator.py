...
class Coordinator:
    ...
    def run(self):
        if random.random() > 10/10:  # simulate a crash   --------------------- crash in state INIT ----------------
        ...
        if random.random() > 3/3:  # simulate a crash   -------------------------crash in state WAIT ----------------
            self._enter_state('ABORT')
        ...
        if random.random() > 0 / 3:     # simulate a crash   -------------------------crash in state PRECOMMIT ---------
            self._enter_state('ABORT')
        ...
        while len(participants_list) > 0:
            ...
            if msg:
                ...
                participants_list.remove(msg[0])
